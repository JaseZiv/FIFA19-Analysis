...
Plotting the relationship below, it can be seen that as players age, their valuations tend to increase up to their early 30s, and then begin declining in the years of age between 31-35, and then rapidly decline for players older than 35.
```{r, message=FALSE, warning=FALSE, fig.width=12, fig.height=8}
fifa_data %>%
...
As expected, Forwards and Midfieldrs are going to cost you more than Defenders and Goalkeepers.
...
Reading Soccernomics, a key thread of successful clubs was they knew when to get rid of players when a potential suitor was willing to pay for a player much more than what they were worth.

This section will look to predict the price of players, and then determine which players should be sold, and which should be purchased relative to their valuation.
...
value_rating_lm <- lm(ValueNumeric_pounds  ~ Overall + AgeGroup + PositionGroup + `International Reputation` + WageNumeric_pounds, data = model_data)
...
>>>>>>> 8bda59eab4413c3c1d6fbae2a1294f47b3bc3d18
