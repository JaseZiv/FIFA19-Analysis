...
## Correlation Matrix for all except GK

The below correlation matrix displays the correlation amongst attribute variables for all players except goalkeepers. The Pearson method is used here as Player Value hasn't been included, which is prone to extreme outliers that could skew the results.

The attributes with the highest positive correlation with a player's overall rating for non-GK players are:
...
`Balance` has the lowest correlation with a player's overall rating.
...
test <- fifa_data %>%
  ...
  select(-gk_vars, -ID, -X1, -starts_with("Value"), - `Jersey Number`, -starts_with("Wage")) %>%
  as.matrix() %>%
  na.omit() %>%
  cor() %>% 
...
```{r}
epl <- c('Liverpool','Manchester City','Tottenham Hotspur', 'Chelsea', 'Arsenal','Manchester United','Watford',
         'Wolverhampton Wanderers', 'Leicester City','West Ham United','Everton', 'Bournemouth','Brighton & Hove Albion',
         'Crystal Palace','Southampton','Burnley', 'Newcastle United','Cardiff City','Fulham', 'Stoke City','Huddersfield Town')
...
fifa_data %>%
  filter(PositionGroup != "Unknown") %>%
  ggplot(aes(x= ValueNumeric_pounds, y= WageNumeric_pounds)) +
  geom_point() +
  facet_wrap(~ PositionGroup)


fifa_data %>%
  filter(PositionGroup != "Unknown" | PositionGroup != "GK") %>%
  group_by(Position) %>%
  summarise(n = n(), testCor = cor(Overall, ValueNumeric_pounds)) %>% View()
...
```{r}
model_data <- fifa_data %>%
  filter(PositionGroup != "Unknown") %>%
  filter(PositionGroup != "GK") %>%
  mutate(RoomToGrow = Potential - Overall)


fit_lm <- lm(ValueNumeric_pounds  ~ Overall + AgeGroup + RoomToGrow + `International Reputation` + WageNumeric_pounds + PositionGroup + Special + `Skill Moves`, data = model_data)
...
model_data$PredictedValue <- predict(fit_lm, model_data)

model_data %>%
  select(Name, Club, Age, ValueNumeric_pounds, PredictedValue) %>%
  mutate(PredictionError = PredictedValue - ValueNumeric_pounds) %>%
  arrange(desc(PredictionError)) %>% tail(20)
...
wisc.km <- kmeans(scale(data_cluster), centers = 8, nstart = 20)

diagnosis <- fifa_data %>%
  filter(PositionGroup != "Unknown") %>%
  filter(PositionGroup != "GK") %>% pull(PositionGroup)
...
cluster_analysis %>%
  filter(Cluster == 6) %>%
  arrange(desc(Overall)) %>% head(n=20) %>%
  ggplot(aes(x= reorder(Name, Overall), y= ValueNumeric_pounds)) +
  geom_col() +
  coord_flip()
...
cluster_analysis %>%
  filter(Cluster == 6) %>%
  arrange(desc(Overall)) %>% head(n=30) %>%
  mutate(Under27 = ifelse(Age < 27, "Yes", "No")) %>%
  ggplot(aes(x= Overall, ValueNumeric_pounds)) +
  geom_text(aes(label = paste(Name, "(", Age, ")", sep = ""), colour = Under27), check_overlap = T)
...
cluster_analysis %>%
   group_by(Cluster) %>%
   arrange(desc(Overall)) %>% top_n(n=30) %>%
   mutate(Under27 = ifelse(Age < 27, "Yes", "No")) %>%
   ggplot(aes(x= Overall, y= ValueNumeric_pounds)) +
   geom_text(aes(label = paste(Name, "(", Age, ")", sep = ""), colour = Under27), check_overlap = T) +
