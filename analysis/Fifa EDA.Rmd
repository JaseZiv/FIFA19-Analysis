...
fifa_data <- read_csv("data/data.csv") %>% select(-X1)
...
There a strong positive relationship between the players rating and valuation with a Spearman correlation of `r round(cor(fifa_data$Overall, fifa_data$ValueNumeric_pounds, method = "spearman"),4))`. The Spearman method was used to calculate the correlation because of presence of large outliers. 
...
spearman_cor_with_overall %>% left_join(pearson_cor_with_overall, by = "cor_colnames") %>% 
  rename(Feature = cor_colnames, Spearman = `..x`, Pearson = `..y`) %>% filter(Feature != "Overall") %>% head(10) %>% DT::datatable()
